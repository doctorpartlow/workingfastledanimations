...
void draw()
{
  ...
  { // every 8 seconds it will change the variables
    ...
    separa = sep; // random(3.1, 8.5);
    alignm = ali; // random(1.5,8.5);
    cohesi = coh; // random(1.5,8.5);
    boidseperation = boidsep;//random(4, 10);
    neighbordistance = neidist;//random(4, 10);
    fadebyvalue = fadebyval;//random(1, 8) * 20;
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    drawPixelXYF(boid->location.x, boid->location.y, ColorFromPalette(PartyColors_p, boid->hue * 15, 255, NOBLEND));
