...
// #define NUM_LEDS 576
//CRGB leds[NUM_LEDS];
#define NUM_PARTICLES 200  //set this to the number of particles. the varialbe describes what it's supposed to be. it works with 50 but it's a little slow. on an esp32 it looks pretty nice at that number 15 is a safe number
// const uint8_t kMatrixWidth = 24;
// const uint8_t kMatrixHeight = 24;
 #define WIDTH kMatrixWidth
 #define HEIGHT kMatrixHeight
...
//     byte hue = 0;
 ...
 Boid boids[NUM_PARTICLES];    //this makes the boids
  ...
  for (int i = 0; i < NUM_PARTICLES; i++) {
    ...
    //boid.mass = 1;//random(0.1,2); // random(0.1, 2);
...
class Attractor {
  ...
  Attractor() {
    location = PVector(ROWS / 2, COLS / 2);
    mass = 6;
    G = .5;
  }
...
void draw(){
 ...
 for (int i = 0; i < NUM_PARTICLES; i++) {
    ...
    boid.update();
    drawPixelXYF(boid.location.x, boid.location.y, ColorFromPalette(LavaColors_p, boid.hue, 255, LINEARBLEND));
    boids[i] = boid;
